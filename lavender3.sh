...
KERNELDIR=$PWD
TOOLDIR=/root/android_kernel_xiaomi_lavender/
...
STRIP="${TOOLCHAIN64}/aarch64-linux-android-4.9/bin$(echo "$(find "${TOOLCHAIN64}/aarch64-linux-android-4.9/bin" -type f -name "aarch64-*-gcc")" | awk -F '/' '{print $NF}' |\
			sed -e 's/gcc/strip/')"
...
# Export
PATH="${TOOLDIR}/android_prebuilts_clang_host_linux-x86_clang-5696680/bin:${TOOLCHAIN64}/aarch64-linux-android-4.9/bin:${PATH}:${TOOLCHAIN32}/arm-linux-androideabi-4.9/bin:${PATH}"
